(ns gojek.main
  "Kotodama WASM entrypoint for the Gojek clean-room actor (L4) — Clojure port.
  ...
  facts (`gojek.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"gojek.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "gojek-compat" :tier tier :entities entities} 200])
